                ___  ____   __       _  _  _____  __   ____  2
               / __)(  _ \ /__\  ___( \/ )(  _  )(  ) (_  _)
               \__ \ )___//(__)\(___)\  /  )(_)(  )(__  )(
               (___/(__) (__)(__)     \/  (_____)(____)(__)
                                R E A D M E
                    Version 1.3, from September 11th 2022

  S U M M A R Y
════════════════════════════════════════════════════════════════════════════
Track Name:         Spa-Volt 2
Folder Name:        spavolt2
Author:             Kiwi
Creation:           March to December 2018
Length:             532 meters
Length R:           509 meters
Difficulty:         Extreme
Reversed Mode:      YES
Practice Star:      YES, 12 ducks
Time Trial Time:    00:39:50
Time Trial Time R:  00:39:00
Racing Stars:       2 ducks


  T R A C K    D E S C R I P T I O N
════════════════════════════════════════════════════════════════════════════
This is an alternate version of the first Spa-Volt.

Horray, the storm is over! The terrible nightly thunderstorm caused a lot
of damage to the environment, so the Spa kept closed the next day. This is
the perfect chance for having another RC-car race at Spa-Volt! :)

It was a sunny and warm day in the beginning of the autumn season. The sun
is going down in the next hour and throwing long shadows over the bathing
area. Crickets are using the chance for chirping maybe for the last time
this year. Crows are circeling above the Spa, searching for crickets for
their dinner. Who is the fastest in this most romantic and peaceful place
on earth?

Differences to Spa-Volt 1:

+ Weather: Sunny
+ Daytime: Evening
+ Surfaces: Still wet from the storm last night
+ Raceline: Much faster
+ Difficulty: A little bit easier
+ Also: Discover completly new areas!

Major features of Spa-Volt 2:

+ Fast but challenging raceline
+ Alternate routes
+ Reverse mode
+ Special starting position
+ A lot of customizations (sounds, moving objects...)
+ Can you collect all the 12 practice mode ducks?
+ 2 racing ducks waiting for a Quack-attack!
+ Time trial times to beat
+ More eastereggs to find

Have fun! :)
  Kiwi

  
  I N F O R M A T I O N S
════════════════════════════════════════════════════════════════════════════
+ This track is only compatible and tested with RVGL
+ For best performance it's highly recommended to use RVGL's Shader Version
+ I got the idea for this track during an IRL Spa visit in February 2018
+ There is a forum thread with a creation diary, which can be found here:
  http://forum.re-volt.io/viewtopic.php?f=12&t=151

  
  K N O W N    I S S U E S
════════════════════════════════════════════════════════════════════════════
+ Possible graphical issues when using Non-Shader Version of RVGL,
  especially with mirroring effects and transparency.
+ Sometimes spark effects don't appear at all. This has something to do
  with a limitation of the amount of spark effects visible at the same
  time.
+ Big advantage in lap No. 1 in Time Trial mode, caused by the track
  concept.

  
  C R E A T I O N
════════════════════════════════════════════════════════════════════════════
Tools I've used:    + Blender 2.79b with Marv's Re-Volt Plugin
                    + Ulead Photo Impact 12
                    + Paint.NET 4.1.5
                    + World Cut 11-11-11 by jigebren
                    + prm2hul 10-10-15 by jigebren
                    + mkmirror by Ali
                    + Audacity 2.2.2
                    + Notepad++ v7.9.1
                    + RVGL Makeitgood mode

W/PRM Polygons:     ~ 65.000
NCP Polygons:       ~ 38.000
Instance Models:    63
Total Instances:    ~ 820

AI-nodes (spavolt2.fan) for the regular version were made by Wichilie.
AI-nodes (reversed/spavolt2.fan) for the R-version were made by Mr. Burguers.
ViziBoxes (spavolt2.vis) were made by Mr. Burguers.
Cam-nodes (spavolt2.cam) were made by L!lmexican.

I used some meshes and textures from other tracks as a basis, and adjusted
them so they fitted my needs. They are listed below. Meshes and textures
which are not listed are made by me from scratch or are modified versions 
from free sources on the internet. Re-used models from the original Re-Volt 
tracks are also not listed.

Used meshes:        + Bridge: Holiday Camp by Romeo
                    + Crow: Fiddlers on the roof by Marv and R6te
                    + Restaurant Table and Chair: Venice by Gabor
                    + Water dispenser: Quake! by Gabor
                    + Plant in pot: Grisville by Allan1
                    + Stone: Sakura by Human
                    + Umbrella (Open): Isola Verde by Crescviper
                    + Duck: Venice Sonata by Hil
                    + Ceiling Fan: Jailhouse Rock by Human
                    + Lounge Lamps: Grisville by Allan1
                    + Lounge Chair, Sofa: Grisville by Allan1
                    + Gate components: Holiday Camp by Romeo

Used textures:      + Water dispenser: Quake! by Gabor
                    + Lounge Chair, Sofa: Grisville by Allan1
                    + Gate components: Holiday Camp by Romeo
                    + Landscape pictures on walls: Studio Ghibli artworks
                    + Kebab-O-Mat logo: Provided by Zeino
                    + RC-car pictures in Restaurant: Re-Volt gallery
                    + Pemto textures: Taken from the PPP-Pack
                    + Skybox: Red Rock Valley by Saffron

Used Sounds:        + animal1.wav               Ranch by Marv & R6te
                    + birds1.wav                *1
                    + birds2.wav                *1
                    + birds3.wav                *1
                    + bottle.wav                Acclaim
                    + creak.wav                 *1
                    + helicopter_loop.wav       *3
                    + kids.wav                  *2
                    + plane.wav                 SILENCE
                    + sdrsclos.wav              *3
                    + sdrsopen.wav              SILENCE
                    + starfire.wav              Acclaim, *2
                    + stream.wav                *1
                    + tv.wav                    *1
                    + wash.wav                  *2

*1: https://bigsoundbank.com
    Licensed under a Creative Commons "Attribution" 4.0 Licence
*2: http://soundbible.com
    Licensed under a Attribution-NonCommercial 3.0 International License
*3: http://sounddogs.com
    Unknown licence type

Used Fonts:         + Spa-Volt logo: Park Avenue PT, by Tilde

Used Music:

"Voyage to Pluto (Dreamworld remix)" by Prism
Label: Toucan Music
http://freemusicarchive.org/
Licensed under a Attribution-NonCommercial 3.0 International License


  T H A N K    Y O U
════════════════════════════════════════════════════════════════════════════
Thank you to the original Probe developers for creating Re-Volt, and thank
you to Huki and Marv for keeping the game alive nearly 20 years after its
initial release by still improving and maintaining RVGL.

Thank you to the great Re-Volt community, for inspiring and motivating
me. Also big thanks to the ladies and gentlemen at the Discord's #tracks
channel, for your never-ending help for a track creator newbie like me.

Thank you to Gabor, Human and Hil, whose tracks inspired me a lot.

Thank you to my wife Bianca, who had to spend hundreds of evenings without
me, because I was working on Spa-Volt instead. ;-)

                     ----------------------------
                V E R Y   S P E C I A L    T H A N K S
                     ----------------------------

+ Wichilie          For doing the AI-nodes for the regular version
+ Mr. Burguers      For doing the ViziBoxes and AI-nodes for R-version
+ L!lmexican        For doing the Cam-nodes
+ ZipperZbieracz	For Version 1.22 suggestions and testing

Makeitgood and Blender help: Instant, Gotolei, Kipy, Marv, Mr. Burguers

Beta-Testing: 607, Floxit, Laggeerok, Mighty Cucumber, Man DX, Mr. Burguers,
              Shara, Stvictorar, Wichilie


  R E L E A S E   H I S T O R Y
════════════════════════════════════════════════════════════════════════════
Version 1.0 from December 23rd, 2018
 + Initial public release, released on revoltzone.net
 
Version 1.1 from August 12th, 2020
 + Adjustment release, updated on revoltzone.net
 + Changelog:
       - New skybox
	   - New track preview picture
	   - Changed the cars numberplates to nhood1 ones
	   - Slightly adjusted the starting position
	   - Slightly adjusted some SFX
	   - Increased overall level brightness
	   
Version 1.2 from July 15th, 2021
 + Adjustment release, updated on revoltzone.net and revoltworld.net
 + Changelog:
       - Fixed early finish issue
	   - Fixed some repo bugs
	   - Blocked some alternate paths
	   - Adjusted pickup placements
	   - Adjusted duck (star) positions
	   - Adjusted some instance and object positions
	   - Added some more instances
	   - Adjusted drainage corrugation
	   - Removed water effects at puddles
	   - Slightly adjusted mirror effects
	   - Adjusted some lights
	   - Adjusted overall level brightness
	   - Added sweepcam effect
	   
Version 1.21 from September 1st, 2021
 + Adjustment release, updated on revoltzone.net and revoltworld.net 
 + Changelog:
 	   - Adjusted startpostion to avoid sweepcam bug
 	   - Adjusted some instance and object positions
	   - Adjusted overall level brightness
	   - Adjusted skybox brightness
	   
Version 1.22 from January 28th, 2022
 + Adjustment release, updated on revoltworld.net 
 + Changelog:
	   - Fixed some repo bugs
	   - Added some more instances
 	   - Adjusted some instance and object positions
	   - Removed alternate path before the finish
	   - Adjusted some pickup placements
	   - Added some more lights
	   - Implemented "Pickup Spawncount" feature
	   
Version 1.3 from September 11th, 2022
 + Bugfix release, updated on revoltworld.net 
 + Changelog:
	   - Fixed some repo bugs

 
  D I S C L A I M E R
════════════════════════════════════════════════════════════════════════════
You are free to distribute this track, but please don't change any parts
of it without my approval. Contact me on Discord or at TRH forum.

You are free to use parts of the tracks (models, textures, sounds, ...)
for your own purposes, but don't forget to credit the original authors.